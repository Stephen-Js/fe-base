...
import { VoiceInputProps, VoiceInputStatusType, VoiceInputStatus } from './types'
import './index.scss'
...
export function VoiceInput({
  ...
  const [status, setStatus] = useState<VoiceInputStatusType>(VoiceInputStatus.IDLE)
  ...
  const startRecording = async () => {
    if (disabled || status !== VoiceInputStatus.IDLE) return
    ...
    try {
      ...
      setStatus(VoiceInputStatus.RECORDING)
      setIntermediateText('')
      ...
      setStatus(VoiceInputStatus.IDLE)
  ...
  const stopRecording = async () => {
    if (status !== VoiceInputStatus.RECORDING) return

    clearTimer()
    setStatus(VoiceInputStatus.PROCESSING)
    setIntermediateText('')
  ...
  return (
    <View className={`voice-input ${className}`}>
      <View
        className={`voice-input__btn ${status === VoiceInputStatus.RECORDING ? 'recording' : ''} ${disabled ? 'disabled' : ''}`}
...
export type { VoiceInputProps, VoiceInputStatusType } from './types'
